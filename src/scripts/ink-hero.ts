...
const hero = document.querySelector<HTMLElement>("[data-ink-hero]");
  ...
  if (hero && !hero.hasAttribute("data-hero-ready")) {
    ...
    const canvas = hero.querySelector<HTMLCanvasElement>("[data-rain-canvas]");
    ...
    if (stage && canvas && context) {
      ...
      hero.setAttribute("data-rain-renderer", "canvas");
      ...
      const drawDrop = (drop: Drop) => {
        if (drop.delay > 0 || drop.y < -drop.length || drop.y > drop.impactY) return;
        const opacity = drop.opacity;
        const tailX = drop.x - drop.directionX * drop.length;
        const tailY = drop.y - drop.directionY * drop.length;

        context.lineCap = "round";

        const softTrail = context.createLinearGradient(tailX, tailY, drop.x, drop.y);
        softTrail.addColorStop(0, "rgba(34, 36, 33, 0)");
        softTrail.addColorStop(.48, `rgba(34, 36, 33, ${opacity * .06})`);
        softTrail.addColorStop(1, `rgba(34, 36, 33, ${opacity * .18})`);
        context.lineWidth = drop.width * 2.15;
        context.strokeStyle = softTrail;
        context.beginPath();
        context.moveTo(tailX, tailY);
        context.lineTo(drop.x, drop.y);
        context.stroke();

        const inkTrail = context.createLinearGradient(tailX, tailY, drop.x, drop.y);
        inkTrail.addColorStop(0, "rgba(27, 29, 27, 0)");
        inkTrail.addColorStop(.32, `rgba(27, 29, 27, ${opacity * .22})`);
        inkTrail.addColorStop(.72, `rgba(27, 29, 27, ${opacity * .72})`);
        inkTrail.addColorStop(1, `rgba(27, 29, 27, ${opacity})`);
        context.lineWidth = drop.width;
        context.strokeStyle = inkTrail;
        context.beginPath();
        context.moveTo(tailX, tailY);
        context.lineTo(drop.x, drop.y);
        context.stroke();
        ...
        if (drop.layer !== "far") {
          const highlightOffset = drop.layer === "near" ? .78 : .56;
          const offsetX = drop.directionY * highlightOffset;
          const offsetY = -drop.directionX * highlightOffset;
          const highlight = context.createLinearGradient(tailX, tailY, drop.x, drop.y);
          highlight.addColorStop(0, "rgba(252, 251, 247, 0)");
          highlight.addColorStop(.56, `rgba(252, 251, 247, ${opacity * .08})`);
          highlight.addColorStop(1, `rgba(252, 251, 247, ${opacity * .34})`);
          context.lineWidth = Math.max(.38, drop.width * .3);
      ...
      const drawRain = (entryOpacity = 1) => {
        context.clearRect(0, 0, width, height);
        if (!rainStarted || entryOpacity <= 0) return;
        context.save();
        context.globalAlpha = entryOpacity;
      ...
      const frame = (time: number) => {
        ...
        applyScrollProgress();
        hero.style.setProperty("--mist-ambient", `${Math.sin(time * .00023) * 10}px`);

        if (!rainStarted && time - enteredAt > 620) {
          rainStarted = true;
          rainStartedAt = time;
        }
        if (rainStarted && !manuallyPaused) updateRain(delta, time);
        const entryOpacity = rainStarted ? clamp((time - rainStartedAt) / 260) : 0;
        drawRain(entryOpacity);
      ...
      const resizeCanvas = () => {
        ...
        const pixelRatio = Math.min(window.devicePixelRatio || 1, mobileQuery.matches ? 1.35 : 1.8);
