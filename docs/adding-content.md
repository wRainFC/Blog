...
Windows 也可以双击 `content-studio.cmd`。工作台只监听本机地址，打开后访问 `/studio`；它直接写入当前仓库，不会自动提交 Git 或发布网站。

工作台支持创建和编辑课程、课程笔记、随笔的 `.md` 文件，并提供字段校验、Markdown/公式/Callout 预览、草稿状态和图片上传。保存后会自动运行 `pnpm check`，检查结果会显示在底部面板。
